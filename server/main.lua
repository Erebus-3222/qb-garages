...
QBCore.Functions.CreateCallback("qb-garage:server:GetGarageVehicles", function(source, cb, garage, type, category)
    ...
    if type == "public" then        --Public garages give player cars in the garage only
        local garagedb
        if Config.GlobalPublic then
            garagedb = "public"
        else
            garagedb = garage
        end
        MySQL.query('SELECT * FROM player_vehicles WHERE citizenid = ? AND garage = ? AND state = ?', {pData.PlayerData.citizenid, garagedb, 1}, function(result)
            if result[1] then
                cb(result)
            else
                cb(nil)
    ...
    elseif type == "depot" then    --Depot give player cars that are not in garage only
        MySQL.query('SELECT * FROM player_vehicles WHERE citizenid = ? AND (state = ? OR (state = ? AND depotprice != 0))', {pData.PlayerData.citizenid, 0, 2}, function(result)
            local tosend = {}
            if result[1] then
...
QBCore.Functions.CreateCallback("qb-garage:server:validateGarageVehicle", function(source, cb, garage, type, plate)
    ...
    if type == "public" then        --Public garages give player cars in the garage only
        ...
        MySQL.query('SELECT * FROM player_vehicles WHERE citizenid = ? AND garage = ? AND state = ? AND plate = ?', {pData.PlayerData.citizenid, garagedb, 1, plate}, function(result)
            if result[1] then
                cb(true)
            else
                cb(false)
    ...
    elseif type == "depot" then    --Depot give player cars that are not in garage only
        MySQL.query('SELECT * FROM player_vehicles WHERE citizenid = ? AND (state = ? OR (state = ? AND depotprice != 0)) AND plate = ?', {pData.PlayerData.citizenid, 0, 2, plate}, function(result)
            if result[1] then
                cb(true)
            else
                cb(false)
...
QBCore.Functions.CreateCallback("qb-garage:server:checkOwnership", function(source, cb, plate, type, house, gang)
    ...
    elseif type == "house" then     --House garages only for player cars that have keys of the house
        MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', {plate}, function(result)
            if result[1] then
                -- local hasHouseKey = exports['qs-houses']:hasKey(result[1].license, result[1].citizenid, house)
                -- if hasHouseKey then
                    cb(true)
                -- else
                --     cb(false)
            ...
            if state == 0 or state == 1 or state == 2 then                                          --Check state value
                if type ~= "house" then
                    if Config.Garages[garage] then                                                             --Check if garage is existing
                        local garagedb
                        if Config.GlobalPublic and type == "public" then
                            garagedb = "public"
                        else
                            garagedb = garage
                        end
                        MySQL.update('UPDATE player_vehicles SET state = ?, garage = ?, fuel = ?, engine = ?, body = ? WHERE plate = ?', {state, garagedb, fuel, engine, body, plate})
                    end
                else
